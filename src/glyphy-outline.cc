			...
			unsigned int 		start)
{
  for (unsigned int i = 1; i < start; i++) {
    ...
    if (current_arc.intersects_arc (a) != Point (GLYPHY_INFINITY, GLYPHY_INFINITY)) {
      ...
      return current_arc.intersects_arc (a); //true;      
    }
  } 
				      ...
				      glyphy_bool_t          inverse)
{
  ...
  for (unsigned int i = 1; i < num_endpoints; i++) {
    ...
    const glyphy_arc_endpoint_t enext = endpoints[i];    
    Arc current_arc (ethis.p, enext.p, enext.d);
    Point intersection = arc_intersects_contour (current_arc, endpoints, start);
    if (intersection != Point (GLYPHY_INFINITY, GLYPHY_INFINITY))
      printf("intersects the arc [(%f,%f), (%f,%f), %f], center (%f,%f), radius=%f at the point (%f,%f).\n", current_arc.p0.x, current_arc.p0.y, current_arc.p1.x, current_arc.p1.y, current_arc.d, current_arc.center ().x, current_arc.center ().y, current_arc.radius (), intersection.x, intersection.y);
  ...
  // glyphy_remove_overlaps (*endpoints, num_endpoints);
...
/* Returns true if outline was modified 
glyphy_bool_t
glyphy_remove_overlaps (glyphy_arc_endpoint_t *endpoints,
			unsigned int           num_endpoints)
{

  unsigned int first_contour_end = 0;
  bool ret = false;
  ...
  for (unsigned int i = 1; i < num_endpoints; i++) {
    ...
    // Add the first contour completely.
    
    // For each new contour:
    
    // Try adding edges; either find an intersection with old contour, or return to start.
    
    // If there is an intersection, pick the outside side.
    
    // [*] Start adding edges in new contour along this direction.
    
    // Add edges until we find a new intersection.
    
    // Trace the original contour, deleting edges, until we find a new intersection.
    
    // If the new intersection is the starting intersection, then yay. 
    
    // Otherwise, return to [*]. 
    
    if (arc_intersects_contour (current_arc, endpoints, first_contour_end))
